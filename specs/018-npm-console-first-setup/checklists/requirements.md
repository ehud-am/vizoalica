...
- **Decisions made by default rather than asked** (see Assumptions), worth confirming before planning:
  1. **Roles are credentials, enforced by the backend.** Analysts get a read-only key and website owners get a key that can manage projects and websites within a scope,
     revised from "no new credential for owners". Per-website signing keys, which
