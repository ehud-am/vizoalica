...
**Primary Dependencies**: Browser SDK with no runtime dependency requirement; backend HTTP service; JOSE-compatible JWT verification; JSON Schema validator; CloudEvents-compatible event envelope; OpenAPI for HTTP contracts

**Storage**: Durable event store for accepted events plus lightweight configuration store for projects, sources, public identifiers, signing keys, and quota policies. Initial implementation may use one database, but ingestion must preserve a boundary that allows queued/batched storage later.
...
**Constraints**: Browser integration must fail silently; no long-lived secrets in the browser; signed ingest tokens required for production mode; public-ID unsigned ingestion allowed only for explicitly marked demo/dev mode; strict event/payload limits before expensive work; no raw form content or sensitive URL query values stored by default.
...
See [research.md](./research.md). Key decisions: CloudEvents JSON batch envelope, JSON Schema validation, JWT/JOSE short-lived ingest tokens, W3C Trace Context compatibility, optional IAB TCF consent propagation, and container-first self-hosting.
...
- **Self-Hosted, Low-Cost Operations**: PASS — quickstart validates local self-hosted operation and quota behavior.
